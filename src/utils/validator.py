...
class FieldValidator:
    ...
    def validate_player_names(cls, player1_name: str, player2_name: str):
        ...
        if player1_name == player2_name:
            raise ServiceValidationError(
                message=f"Player can't play with himself"
            )

    @staticmethod
    def validate_name(name: str, field: str = "Player name"):
        FieldValidator.validate_string_only_english_letters(value=name,
                                                            field_name=field)
        FieldValidator.validate_string_field_less_than_len(value=name,
                                                           length=50,
                                                           field_name=field)
    ...
    def validate_string_field_less_than_len(value: str, length: int, field_name: str):
        if len(value) > length:
            raise ServiceValidationError(
                message=f"{field_name} length must be less than {length}"
            )
